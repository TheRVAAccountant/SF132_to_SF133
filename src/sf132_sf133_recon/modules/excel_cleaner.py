"""
Excel data cleaner module.

This module provides functions for cleaning external data connections and references
in Excel files, improving security and preventing potential corruption.
"""
...
class ExcelDataCleaner:
    ...
    def clean_external_references_openpyxl(self, workbook):
        """
    ...
    def _clean_worksheet_external_data(self, worksheet):
        """
        ...
        try:
            ...
            if hasattr(worksheet, 'data_validations'):
                try:
                    ...
                    # Handle older versions of openpyxl
    ...
    def clean_external_references_com(self, file_path: str) -> bool:
        ...
        """
        if not WINDOWS_COM_AVAILABLE or not sys.platform.startswith('win'):
            self.logger.warning("COM operations not available on this platform")
            return False
    ...
    def clean_sheet_data(self, sheet) -> None:
        """
...
def clean_excel_external_data(file_path: str, logger_instance=None) -> bool:
    """
    ...
    cleaner = ExcelDataCleaner(logger_to_use)
    
    # First try with COM which is most thorough (Windows only)
    if WINDOWS_COM_AVAILABLE and sys.platform.startswith('win'):
    ...
    try:
        ...
        logger_to_use.info("Completed openpyxl external references cleanup")
        return True
    except Exception as e:
        logger_to_use.warning(f"Error during openpyxl cleanup: {e}")
        return success